...
option_list=list( 
  make_option(c("-i", "--input"), dest = "file_name", type = "character", 
              help = "REQUIRED: Input file name that stores the gene expression data matrix that users want to construct gene co-expression network and do clustering. Each row corresponds to a gene, and each column corresponds to a replicate. The column names are replicate names. The row names are gene names."), 
  ...
  make_option(c("-c", "--corr_thld"), type = "double", default = 0.75,
              help = "Maximum threshold to set elements of Pearson correlation matrix to be zero when method=’simple’. [default %default]"), 
  make_option(c("-q", "--p_thld"), type = "double", default = NULL,
              help = "Maximum threshold for p from Pearson correlation when method=’simple’. [default %default]"), 
  ...
  make_option(c("-t", "--type"), dest = "output_type", type = "character", default = 'edge', 
              help = "Type of output. When method = ‘edge’ or ‘e’, the function outputs edge list of gene co-expression network consisting of gene pairs with Pearson correlation greater than corr_thld. When method = “adjmat” or ‘a’, the function outputs the adjacency matrix of constructed gene co-expression network. [default \"%default\"]"), 
