...
sub call_method {
    ...
    my $ctx = PhispyServerContext->new(client_ip => $self->_plack_req->address);
    
    my $args = $data->{arguments};

    # Service Phispy does not require authentication.
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service Phispy.");
    }
